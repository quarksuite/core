...
Using the module for data composition follows its own logical flow:

+ Preset any actions
+ Create useful processes
+ Assemble data pipelines

Generated data collections also gain some extra support:

+ Propagate a process through each collection value
+ Delegate processes selectively to collection values

** Modules

The first thing to do is import =workflow.js=, so let's do that now.

#+BEGIN_SRC js :tangle "../quarksuite:examples/handbook/advanced-workflow/refactoring/tokens.js"
import {
  convert,
  adjust,
  harmony,
