...
Quarks System Core is written as a literate program in a top-down structure. You can access the test
suite from =index.html=.
...
=output_css=: CSS custom properties
=output_scss/output_less/output_styl=: Variables for the respective preprocessor
=output_raw/output_yaml=: exports the raw dictionary (as JSON or YAML)
=output_gpl=: extracts the color dictionary for use in GIMP/Inkscape
=output_sketchpalette=: extracts the color dictionary as a Sketch palette
