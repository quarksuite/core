...
- [[#formatters][Formatters]]
  - [[#setting-up-formatter-helpers][Setting Up Formatter Helpers]]
...
This document contains the source code of Quarks System Core written in a literate programming
style. The main reason I opted to write this way is that, as of v1, QSC now explicitly builds off
layers of abstraction and very /specific/ kinds of functional composition.

Hopefully these decisions make the right workflows easier for you to accomplish and errors harder to
come by. I invite you to submit an issue if you hit a bump.
...
The entry point of the entire library is =mod.js=. Importing this will give you the entire kit to
play with and open up any approach to using it. The main time to use the entry point is during the
development of your tokens.

When you're ready for production, you'll want to narrow your imports to the layers you used. This
will reduce the overall size of your system.
...
+ Quarks System Dictionary: an object representing the design token schema. A standard form is
  generated by the bootstrapper, but just about any design token taxonomy is allowed.
+ Formatters: transforms the schema into a target format (e.g. CSS custom properties, Sass
  variables)
+ Bootstrapper: outputs a standard design token schema with batteries included. Constructed from formulas
+ Formulas: schema category helpers. May or may not be constructed from configurations
+ Configurations: schema data helpers. May or may not be constructed from utilities
+ Utilities: power tools for building and transforming data directly

** Rules

The system design also lays down a set of ground rules intended to aid understanding and use of all
these moving parts.

1) Utilities and formatters are always =snake_case=
2) Utilities are always named by the data type they affect (=color_to_hex=, =ms_create=)
3) The bootstrapper, formulas, and configurations are always =PascalCase=
...
By default, the =Quarks= bootstrapper outputs a standard option schema ideal for grayscale prototyping
and filled with data that's about as complete as you can get for small to medium web projects.

The best way to understand the schema: don't pay attention to the naming, pay attention to the /structure/.

#+NAME: bootstrapper-defaults
#+begin_src js
const dict = {
  color: {
    a: {
  ...
  text: {
    family: {
      body: "-apple-system, BlinkMacSystemFont, avenir next, avenir, helvetica neue, helvetica, Ubuntu, roboto, n...",
      headings: "Iowan Old Style, Apple Garamond, Baskerville, Times New Roman, Droid Serif, Times, Source Serif Pro,...",
  ...
  animation: {
    duration: {
      base: "1000ms",
      interval: [ "750ms", "658.25ms", "551.2ms", "440.88ms", "346.29ms", "284.5ms" ],
      fastest: "250ms"
    },
    easing: {
      x: [
...
const completeQSD = {
  project: {
    ...
    author: "Ed N. Bacon",
    version: "0.0.0",
    // bump: "minor" (optional autobumping via "major", "minor", "patch", "pre", "build")
    license: "Unlicense"
  },
  ...dict
}
...
For the sake of focus, I'm only going to use subsets of the data for examples.
...
#+BEGIN_SRC js
console.log(dict.color.a["500"]) // #7d7d7d
console.log(dict.text.size.x3) // 2.25rem
console.log(dict.grid.columns) // 6
#+END_SRC
...
#+BEGIN_SRC js
console.log(
  dict.text.leading.narrow, // [1.375, 1.3291, 1.2756, 1.2204, 1.1731, 1.1423]
  dict.text.leading.narrow[0], // 1.375
  dict.text.leading.narrow[3] // 1.2204
)
#+END_SRC
...
within are treated as variants. The significance of this won't be evident until we reach formatters,
...
about your data that the formatters will use if supported. Multiline metadata is allowed, so add as
much info as neccessary.
...
const withMeta = {
  project: {
    ...
    license: "Unlicense",
    metadata: {
     description: "Project can have a description",
     comments: "Project can also have some useful comments"
   }
  },
  color: {
    metadata: {
     description: "Individual categories can also have a description",
     comments: "Individual categories can have useful comments",
   },
    // ...tokens
  }
}
...
Another idea central to design token authorship is the distinction between a broad pool of option
tokens and narrowing by the needs of your project down to decision tokens.

The bootstrapper excels at providing a breadth of options that you can then drill down to only what
you use.
...
const schema = {
  project: completeQSD.project,
  color: {
    light: dict.color.a["400"],
    lighter: dict.color.a["200"],
    bg: dict.color.a["50"],
    dark: dict.color.a["500"],
    darker: dict.color.a["700"],
    fg: dict.color.a["900"]
  },
  text: {
    primary: {
      family: dict.text.family.body,
      regular: dict.text.weight.body.regular,
      bold: dict.text.weight.body.bold,
      size: dict.text.size.base,
      leading: dict.text.leading.base,
      measure: dict.text.measure.base
    },
    secondary: {
      family: dict.text.family.headings,
      bold: dict.text.weight.headings.bold,
      size: {
        headline: dict.text.size.x5,
        subheading: {
          base: dict.text.size.x2,
          x2: dict.text.size.x3,
          x3: dict.text.size.x4
        }
      }
    },
    source: {
      family: dict.text.family.code,
      regular: dict.text.weight.code.regular,
      bold: dict.text.weight.code.bold,
      leading: dict.text.leading.narrow[1]
    },
  }
}
...
#+BEGIN_QUOTE
For security reasons, the formatters do not assume read/write access to your computer. They either
output formatted strings or raw transformed data that you can then write to a file with the method
of your choice.
#+END_QUOTE

Formatters are special utilities that transform a Quarks System Dictionary for use in various
environments and formats. Every formatter walks through the data tree recursively and only cares
about the /type/ of data found. Specifically, it processes values, scales, and subcategories. No
matter how deeply they're nested.

Metadata is handled if the format supports it and *outright ignored* otherwise.

The project autoversioning is set up to work on each call of a given formatter. This is to ensure
there are no surprises. If autoversioning misbehaves, you can remove it and rebuild with the correct
explicit version.

** Setting Up Formatter Helpers

First, I import all the helper functions and needed utilities. Then it's down to business.
...
as well as the most popular preprocessors. The included examples are the *real output* of calling
each formatter with the =QSD= data defined above.
 ...
 * COMMENTS: N/A
 * ----------------------------------------------------------------
 * Updated on 10/8/2021 at 12:27:59 PM
 **/
...
:root {
  ...
  --color-darker: #414141;
  --color-fg: #0b0b0b;
  --text-primary-family: -apple-system, BlinkMacSystemFont, avenir next, avenir, helvetica neue, helvetica, Ubuntu, roboto, n...;
  --text-primary-regular: 400;
  --text-primary-bold: 700;
  --text-primary-size: 1rem;
  --text-primary-leading: 1.5;
  --text-primary-measure: 75ch;
  --text-secondary-family: Iowan Old Style, Apple Garamond, Baskerville, Times New Roman, Droid Serif, Times, Source Serif Pro,...;
  --text-secondary-bold: 700;
  --text-secondary-size-headline: 5.0625rem;
  --text-secondary-size-subheading: 1.5rem;
  --text-secondary-size-subheading-x2: 2.25rem;
  --text-secondary-size-subheading-x3: 3.375rem;
 ...
 * COMMENTS: N/A
 * ----------------------------------------------------------------
 * Updated on 10/12/2021 at 11:39:02 AM
 */
 ...
 * COMMENTS: N/A
 * ----------------------------------------------------------------
 * Updated on 10/12/2021 at 11:41:04 AM
 */
 ...
 * COMMENTS: N/A
 * ----------------------------------------------------------------
 * Updated on 10/12/2021 at 11:43:56 AM
 */
...
=project= data is explicitly split off from the =token= data.
...
#+BEGIN_SRC deno :results output code replace :allow read :noweb yes
const { output_raw } = await import(`${Deno.cwd()}/formatters.js`);

<<decision-tokens>>

console.log(output_raw(QSD))
#+END_SRC
...
# Updated on 10/12/2021 at 12:02:33 PM
...
# COMMENTS: N/A
#
# Updated on 10/12/2021 at 12:04:52 PM

Columns: 6
151	151	151	LIGHT (#979797)
198	198	198	LIGHTER (#c6c6c6)
248	248	248	BG (#f8f8f8)
125	125	125	DARK (#7d7d7d)
...
#+begin_src json
{"colors":[{"red":0.592156862745098,"green":0.592156862745098,"blue":0.592156862745098,"alpha":1},{"red":0.7764705882352941,"green":0.7764705882352941,"blue":0.7764705882352941,"alpha":1},{"red":0.9725490196078431,"green":0.9725490196078431,"blue":0.9725490196078431,"alpha":1},{"red":0.49019607843137253,"green":0.49019607843137253,"blue":0.49019607843137253,"alpha":1},{"red":0.2549019607843137,"green":0.2549019607843137,"blue":0.2549019607843137,"alpha":1},{"red":0.043137254901960784,"green":0.043137254901960784,"blue":0.043137254901960784,"alpha":1}],"pluginVersion":"1.4","compatibleVersion":"1.4"}
#+end_src
...
#+BEGIN_QUOTE
TailwindCSS interop makes much more sense to use with a full collection of option tokens as it has
its own tree-shaking behavior to weed out what you don't use. This makes narrowing unnecessary.

As such, the example is generated from the =dict= orginally built by the bootstrapper.

The actual setup may vary depending on your kit.
#+END_QUOTE

As mentioned earlier, the Quarks System Dictionary schema is just a plain JavaScript object that
goes through no special processing. The TailwindCSS data transformer scans the data and changes any
=base= properties it finds to the =DEFAULT= key Tailwind uses. This is enough to make it compatible.

As an example:

#+BEGIN_SRC deno :results output code replace :allow read :noweb yes
const { output_tailwindcss } = await import(`${Deno.cwd()}/formatters.js`);

<<complete-dict>>

const { color, text } = output_tailwindcss(completeQSD);
...
After =QSD= passes through the data transformer, convert it to JSON with =JSON.stringify= and it
will satisfy Style Dictionary's requirement. From there, you can do additional processing.

#+BEGIN_QUOTE
Don't worry about pretty-printing the JSON as it's simply a bridge into Style Dictionary.
#+END_QUOTE

#+begin_src json
{"color":{"light":{"value":"#979797"},"lighter":{"value":"#c6c6c6"},"bg":{"value":"#f8f8f8"},"dark":{"value":"#7d7d7d"},"darker":{"value":"#414141"},"fg":{"value":"#0b0b0b"}},"text":{"primary":{"family":{"value":"-apple-system, BlinkMacSystemFont, avenir next, avenir, helvetica neue, helvetica, Ubuntu, roboto, n..."},"regular":{"value":"400"},"bold":{"value":"700"},"size":{"value":"1rem"},"leading":{"value":"1.5"},"measure":{"value":"75ch"}},"secondary":{"family":{"value":"Iowan Old Style, Apple Garamond, Baskerville, Times New Roman, Droid Serif, Times, Source Serif Pro,..."},"bold":{"value":"700"},"size":{"headline":{"value":"5.0625rem"},"subheading":{"base":{"value":"1.5rem"},"x2":{"value":"2.25rem"},"x3":{"value":"3.375rem"}}}},"source":{"family":{"value":"Menlo, Consolas, Monaco, Liberation Mono, Lucida Console, monospace"},"regular":{"value":"400"},"bold":{"value":"700"},"leading":{"value":"1.3291"}}}}
#+end_src
